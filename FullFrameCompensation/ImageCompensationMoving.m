...
dataPaths{4} = '/mnt/data/Datasets/Features/12-May-12/';
dataPaths{5} = '/mnt/data/Datasets/Features/02-Feb-13/';
...
if(calcVel)
    ...
    fileList = dir([dataPath 'Bags/Processed/straight/' 'smooth*']);
    [T,t] = ParseViconGroundTruth([dataPath 'Bags/Processed/straight/'],fileList,0,1);

    % In the ImageStacks folder, we find a folder for each trial 
    full_dirlist = dir([dataPath '/ImageStacks/straight/smooth*']);
    ...
    v_profile = 0;
    for dirIndex = 4:endDirIndex
        v(dirIndex,:) = CompensateImagesPiecewiseVelocity([dataPath 'ImageStacks/straight/'],outPath,T,t,0.5,offsets,dirIndex);        
    end
end
